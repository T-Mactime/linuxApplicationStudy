...
static int button_open(struct inode *inode, struct file *filp)
{
    ...
    if (0 == button_GPIO_number) {
        printk(KERN_DEBUG "of_get_named_gpio error [%d]\n", button_GPIO_number);
        return -1;
    }
    ...
    if (error < 0) {
        printk(KERN_DEBUG "gpio_request error[ [%d]\n", error);
...
static struct file_operations button_chr_dev_fops = {
    .owner = THIS_MODULE, .open = button_open, .read = button_read, .release = button_release};
...
{
    int error = -1;
    printk(KERN_DEBUG "now init start \n");
    ...
    if (error < 0) {
        printk(KERN_DEBUG "fail to add cdev\n");
        goto add_err;
    }

    class_button = class_create(THIS_MODULE, DEV_NAME);  // 创建类
    device_button = device_create(class_button, NULL, button_devno, NULL,
                                  DEV_NAME);  // 创建设备 DEV_NAME 指定设备名，
